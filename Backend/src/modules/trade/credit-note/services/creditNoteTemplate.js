...
function drawTitleBlock(doc, { title, docNumber, date }) {
  const t = (title || 'CREDIT NOTE').toUpperCase();
  ...
  const meta = [
    docNumber ? `CN NO. ${docNumber}` : null,
    date      ? `DATE: ${date}`       : null,
  ].filter(Boolean).join('          ');
...
export async function buildCreditNotePdf(cn, organization, logoUrl) {
  ...
  return new Promise((resolve, reject) => {
    ...
    drawTitleBlock(doc, {
      ...
      date:      fmtDate(ci.creditNoteDate),
    });
    ...
    sectionHeading(doc, nextNum(), 'Credit Note Information', measureFieldRows(doc, infoRows));
    ...
    const custRows = chunkRows(custFields);
    doc.y += SECTION_GAP;
    sectionHeading(
      doc, nextNum(), 'Customer Details',
      measureFieldRows(doc, custRows) + measureAddressBlock(doc, cust.billingAddress) + measureAddressBlock(doc, cust.shippingAddress)
    );
    fieldGrid(doc, custRows);
    addressBlock(doc, 'Billing Address', cust.billingAddress);
    addressBlock(doc, 'Shipping Address', cust.shippingAddress);
    ...
    if ((cn.reasonForCreditNote || '').toString().trim()) {
      doc.y += SECTION_GAP;
      sectionHeading(doc, nextNum(), 'Reason for Credit Note', measureTextBlock(doc, cn.reasonForCreditNote));
      textBlock(doc, cn.reasonForCreditNote);
    }

    if ((cn.notes || '').toString().trim()) {
      doc.y += SECTION_GAP;
      sectionHeading(doc, nextNum(), 'Notes', measureTextBlock(doc, cn.notes));
      textBlock(doc, cn.notes);
    }

    if ((cn.termsAndConditions || '').toString().trim()) {
      doc.y += SECTION_GAP;
      sectionHeading(doc, nextNum(), 'Terms & Conditions', measureTextBlock(doc, cn.termsAndConditions));
      textBlock(doc, cn.termsAndConditions);
    }
